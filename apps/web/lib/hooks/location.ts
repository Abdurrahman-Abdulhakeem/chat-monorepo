...
const getLocationFromIP = async () => {
  ...
  const reverseGeocode = async (lat:any, lng:any) => {
    try {
      const { data } = await axios.get(`https://api.opencagedata.com/geocode/v1/json?q=${lat}+${lng}&key=${process.env.NEXT_PUBLIC_OPENCAGE_API_KEY}&limit=1`)
  ...
  const detectAndUpdateLocation = async () => {
    try {
      ...
      try {
        ...
        console.log("Precise location failed, falling back to IP:", error.message);
      ...
      if (location) {
        const locationString =
          location.formatted ||
          `${location.city || ""}, ${location.state || ""}, ${location.country || ""}`.replace(
            /^,\s*|,\s*$/g,
            ""
          );
